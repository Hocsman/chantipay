...
export default function NewQuotePage() {
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      if (data.items && Array.isArray(data.items)) {
        const newItems: QuoteItem[] = data.items.map(
          (item: Omit<QuoteItem, 'id'>, index: number) => ({
  ...
  return (
    <MobileLayout title="Nouveau devis" subtitle="Créer un devis" showBottomNav={false}>
          ...
          <CardContent>
            <div className="flex gap-3">
              ...
              <Button
                variant="outline"
                size="icon"
                onClick={() => router.push('/mobile/clients')}
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {helperChips.map((chip) => (
                    ...
                    >
                      ...
                      {selectedChips.has(chip.id) && <Check className="h-3 w-3 ml-1.5" />}
                    </Badge>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-2">
              ...
              {parseFloat(depositPercent) > 0 && (
                <div className="flex justify-between text-primary font-medium pt-2">
                  <span>
                    Acompte à verser ({depositPercent}%)
                  </span>
