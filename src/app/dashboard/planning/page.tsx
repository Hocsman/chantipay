...
import { Calendar, ChevronRight, Lightbulb, Plus, Loader2 } from 'lucide-react'
import { EmptyState } from '@/components/mobile/EmptyState'
import { cn } from '@/lib/utils'

const tabs = ['Aujourd\'hui', 'En retard', 'À venir', 'Terminées'] as const
...
export default function PlanningPage() {
  ...
  const currentTasks =
    ...
    activeTab === 'À venir' ? upcomingTasks :
    doneTasks
  ...
  const getStatusBadge = (status: string, priority: string) => {
    ...
    switch (priority) {
      case 'high':
        return { label: 'Priorité haute', color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' }
      case 'medium':
        return { label: 'Priorité moyenne', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' }
      case 'low':
        return { label: 'Priorité basse', color: 'bg-gray-100 text-gray-700 dark:bg-gray-900/30 dark:text-gray-400' }
      default:
  ...
  return (
    <LayoutContainer>
      <PageHeader
        title="Chantiers"
        description="Gérez vos tâches et planifiez vos chantiers"
      />
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {/* Onglets segmentés */}
            <div className="bg-muted p-1 rounded-xl inline-flex w-full mb-4">
              {tabs.map((tab) => (
                <button
                  key={tab}
                ...
                <>
                  {activeTab === "Aujourd'hui" && (
                    <EmptyState
                      ...
                      description="Profitez de cette journée pour préparer vos prochains chantiers."
                      action={{
                  ...
                  {activeTab === 'À venir' && (
                    <EmptyState
                      ...
                      description="Planifiez vos prochaines tâches pour mieux organiser votre semaine."
                      action={{
                        label: 'Créer une tâche',
                        onClick: () => router.push('/dashboard/tasks/new'),
                      }}
                  ...
                  {activeTab === 'Terminées' && (
                    <EmptyState
                      ...
                      description="Vos tâches terminées apparaîtront ici."
                      variant="colorful"
                    />
                  )}
                </>
                  ...
                  {currentTasks.map((task) => {
                    ...
                    return (
                      <div
                        ...
                        className="bg-card rounded-xl p-4 shadow-sm border cursor-pointer hover:shadow-md transition-all"
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <span className={cn('rounded-full px-3 py-1 text-xs font-medium ml-2 whitespace-nowrap', statusInfo.color)}>
                            {statusInfo.label}
                          </span>
                        </div>
